...
For a Kubernetes host without Docker, use `fastapi-prepare-prod --target ghcr-k8s`
...
`fastapi-prepare-prod --target ghcr-k8s` again on the development machine. It
writes a SHA-pinned `values.generated.yaml`. Copy only this Kubernetes bundle to
the deployment host; do not copy the FastAPI source tree and do not install
Docker there:
...
create the GHCR pull Secret above and the FastAPI `chembience-secrets` Secret.
...
Adapt `values.override.yaml` for this FastAPI deployment before using Helm—for
